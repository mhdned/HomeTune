...
class AdsController extends AdminController
{
    ...
    {
        $advertise = Ads::find($id);
        $gallery = Gallery::where('advertise_id', '=', $id)->get();
        return view('admin.ads.gallery', compact('gallery', 'advertise'));
    }
    public function storeGalleryImage($id)
    {
        ...
        if (!empty($file['tmp_name'])) {
            ...
            $name = date('Y_m_d_H_i_s_') . rand(10, 99);
            $inputs['image'] = ImageUpload::UploadAndFitImage($request->file('image'), $path, $name, 600, 600);
        }
